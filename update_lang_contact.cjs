...
const startIdx = content.indexOf('const translations = ') + 'const translations = '.length;
let endIdx = content.lastIndexOf('};') + 1; // get the closing brace
let jsonStr = content.substring(startIdx, endIdx);

try {
  let translations = JSON.parse(jsonStr);
  ...
  const newContent = content.substring(0, startIdx) + JSON.stringify(translations, null, 2) + content.substring(endIdx);
